...
class InventoryItem(Base):
    """User pantry / inventory tracking"""
    ...
    days_until_expiry = Column(Integer, default=7)
    notes = Column(String)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
def get_inventory_items(db: Session, user_id: int) -> List[InventoryItem]:
    """Return inventory items for a user ordered by expiry"""
    return db.query(InventoryItem).filter(InventoryItem.user_id == user_id).order_by(
        InventoryItem.days_until_expiry.asc()
    ).all()


def get_inventory_item(db: Session, user_id: int, item_id: int) -> Optional[InventoryItem]:
...
def upsert_inventory_item(db: Session, user_id: int, item_data: dict, item_id: Optional[int] = None) -> InventoryItem:
    ...
    if item_id:
        ...
        item = InventoryItem(user_id=user_id, **item_data)
        db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def delete_inventory_item(db: Session, user_id: int, item_id: int) -> bool:
...
def create_notification(db: Session, user_id: int, title: str, message: str, notif_type: str = "system", metadata: Optional[dict] = None) -> Notification:
